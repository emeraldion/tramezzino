<?php
	...
	class IdempotenceTest extends \PHPUnit_Framework_TestCase
	{
		function test_single_element()
		{
			...
			$this->assertEquals(Tramezzino::decode(Tramezzino::encode(array('albero'), '(', '|', ')'), '(', '|', ')'), array('albero'));
		}
		...
		function test_no_common_radix()
		{
			...
			$this->assertEquals(Tramezzino::decode(Tramezzino::encode(array('a', 'b', 'c'), '(', '|', ')'), '(', '|', ')'), array('a', 'b', 'c'));
			$this->assertEquals(Tramezzino::decode(Tramezzino::encode($FIXTURES['ARRAYS']['ALBERO_FIORE'], '(', '|', ')'), '(', '|', ')'),
		...
		function test_common_radix()
		{
			...
			$this->assertEquals(Tramezzino::decode(Tramezzino::encode($FIXTURES['ARRAYS']['ALB_A_ER_GO_O_TO'], '(', '|', ')'), '(', '|', ')'),
			  $sorted);

			$sorted = $FIXTURES['ARRAYS']['PESCA_TORE_H_ERIA_IERA'];
			sort($sorted);
			$this->assertEquals(Tramezzino::decode(Tramezzino::encode($FIXTURES['ARRAYS']['PESCA_TORE_H_ERIA_IERA'], '(', '|', ')'), '(', '|', ')'),
			  $sorted);

			$sorted = $FIXTURES['ARRAYS']['MARE_INA_IO_IA'];
			sort($sorted);
			$this->assertEquals(Tramezzino::decode(Tramezzino::encode($FIXTURES['ARRAYS']['MARE_INA_IO_IA'], '(', '|', ')'), '(', '|', ')'),
			  $sorted);
		}
		...
		function test_mixed_strings()
		{
			...
			// Mixed strings
			$this->assertEquals(Tramezzino::decode(Tramezzino::encode(array_merge($FIXTURES['ARRAYS']['ALB_A_ER_GO_O_TO'],
		...
		function test_isin()
		{
			...
			// A real life example
			$this->assertEquals(Tramezzino::decode(Tramezzino::encode($FIXTURES['ARRAYS']['ISIN_1'], '(', '|', ')'), '(', '|', ')'),
