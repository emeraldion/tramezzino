...
class TramezzinoTest extends \PHPUnit_Framework_TestCase
{
    ...
    function test_encode_common_radix()
    {
        ...
        $this->assertEquals(
            ...
            'pesc(a(|tore)|h(eria|iera))'
        );
        $this->assertEquals(
            Tramezzino::encode($FIXTURES['ARRAYS']['MARE_INA_IO_IA'], '(', '|', ')'),
            'mar(e|i(a|na(|io)|o))'
        );
    }
    ...
    function test_encode_mixed_strings()
    {
        ...
        // Mixed strings
        $this->assertEquals(
            Tramezzino::encode($merged, '(', '|', ')'),
            'alb(a|er(go|o|to))|pesc(a(|tore)|h(eria|iera))'
        );
    }
    ...
    function test_encode_scrambled_input()
    {
        ...
        $this->assertEquals(
            Tramezzino::encode($FIXTURES['ARRAYS']['ALBERO_FIORE'], '(', '|', ')'),
    ...
    function test_encode_isin()
    {
        ...
        // A real life example
        $this->assertEquals(
            Tramezzino::encode($FIXTURES['ARRAYS']['ISIN_1'], '(', '|', ')'),
            $FIXTURES['STRINGS']['ISIN_1']
        );
    }
